...
function renderPerson (data, house, person, path) {
  ...
  let label = '<<table align="center" border="0" cellpadding="0" cellspacing="2" width="4">'
  label += `<tr><td align="center">${name}</td></tr>`

  if (fullname) {
    label +=
      '<tr><td align="center">' +
      `<font point-size="10" color="#aaaaaa">${fullname}</font>` +
      '</td></tr>'
  }
  ...
  if (person.born) {
    label +=
      '<tr><td align="center">' +
      `<font point-size="10" color="#aaaaaa">${person.born} ~ ${person.died || ''}</font>` +
      '</td></tr>'
  }
  label += '</table>>'
